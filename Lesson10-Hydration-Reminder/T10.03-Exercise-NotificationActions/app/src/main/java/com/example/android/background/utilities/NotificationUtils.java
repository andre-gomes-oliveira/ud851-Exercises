...
public class NotificationUtils {
    ...
    private static final int WATER_REMINDER_IGNORE_ACTION_ID = 1337;
    private static final int WATER_REMINDER_COMPLY_ACTION_ID = 4004;
    ...
    public static Action ignoreReminderAction(Context context){
        Intent waterReminderServiceIntent = new Intent(context, WaterReminderIntentService.class);
        waterReminderServiceIntent.setAction(ReminderTasks.ACTION_DISMISS_NOTIFICATION);
        PendingIntent pendingwaterReminderServiceIntent = PendingIntent.getService(
                context,
                WATER_REMINDER_IGNORE_ACTION_ID,
                waterReminderServiceIntent,
                PendingIntent.FLAG_UPDATE_CURRENT);

        Action waterReminderAction = null;
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.KITKAT) {
            waterReminderAction = new Action(
                    R.drawable.ic_cancel_black_24px,
                    "Nope!",
                    pendingwaterReminderServiceIntent
            );
        }

        return waterReminderAction;
    }
    ...
    public static Action drinkWaterAction(Context context){
        Intent waterReminderServiceIntent = new Intent(context, WaterReminderIntentService.class);
        waterReminderServiceIntent.setAction(ReminderTasks.ACTION_INCREMENT_WATER_COUNT);
        PendingIntent pendingWaterReminderServiceIntent = PendingIntent.getService(
                context,
                WATER_REMINDER_IGNORE_ACTION_ID,
                waterReminderServiceIntent,
